...
class Line extends Equatable {
  ...
  factory Line.doubles(double startX, double startY, double endX, double endY) {
    return Line(Vector2(startX, startY), Vector2(endX, endY));
  }
  ...
  Vector2? intersectsAt(Line other) {
    double s = (-dy * (start.x - other.start.x) + dx * (start.y - other.start.y)) / (-other.dx * dy + dx * other.dy);
    double t = (other.dx * (start.y - other.start.y) - other.dy * (start.x - other.start.x)) /
